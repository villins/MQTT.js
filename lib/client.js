...
*  rebind - rebind
*
*  @param {object} options - clientId to connect
*
*  @example client.rebind({clientId: '1234'})
*/
MqttClient.prototype.connect = function(options){
  if(!options.clientId || 'string' !== typeof options.clientId){
    throw new Error('clientId must be exists!');
  }
  this.options = options || {};
...
function(topic, opts, callback) {
  ...
  if ('string' === typeof topic) {
    ...
    // Error!
    console.warn('topic must be a var or Array!');
  }
...
MqttClient.prototype._handlePublish = function(packet) {
  ...
  switch (qos) {
    case 0:
      this.emit('publish', topic, message, packet);
      break;
    case 1:
